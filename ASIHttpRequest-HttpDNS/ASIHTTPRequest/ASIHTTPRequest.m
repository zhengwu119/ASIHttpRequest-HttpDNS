...
static const CFOptionFlags kNetworkEvents =  kCFStreamEventHasBytesAvailable | kCFStreamEventEndEncountered | kCFStreamEventErrorOccurred | kCFStreamEventOpenCompleted;
...
- (void)setURL:(NSURL *)newURL
{
    ...
    if (useHTTPDNS) {
        [domain release];
        domain = [[newURL host] copy];
        NSPredicate *expression = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", @"^(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])((\\.(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])){3}|(\\.(\\d{1,2}|1\\d\\d|2[0-4]\\d|25[0-5])){5})$"];
        [ip release];
        NSString *urlStr = nil;
...
- (void)buildRequestHeaders
{
    ...
    if ([self domain] != nil) {
        [self addRequestHeader:@"HOST" value:[self domain]];
    }
...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (useHTTPDNS) {
            NSDictionary *sslSettings = [NSDictionary dictionaryWithObjectsAndKeys: domain, (__bridge id)kCFStreamSSLPeerName, nil];
            if (![[self readStream] setProperty: sslSettings forKey: (__bridge NSString *)kCFStreamPropertySSLSettings]) {
...
- (void)saveErrorToLogFile{
    ...
    NSString *errorLogPath = [documentPath stringByAppendingString:@"error.log"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:errorLogPath]) {
        [fileManager createFileAtPath:errorLogPath contents:nil attributes:nil];
    }
    
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    CTTelephonyNetworkInfo *networkInfo = [[CTTelephonyNetworkInfo alloc] init];
    BOOL wifi = [reachAbility isReachableViaWiFi];
    NSString *networkType = @"WIFI";
    if (!wifi) {
        networkType = [NSString stringWithFormat:@"%@ %@", [networkInfo.subscriberCellularProvider carrierName], [networkInfo currentRadioAccessTechnology]];
    }
    
    /* tm= 时间 ev= 事件 ver= 版本号 ex= 扩展信息 nt= 网络类型*/
    NSDictionary *errLog = @{@"url" : self.url == nil ? @"" : self.url,
    ...
    @synchronized(self) {
        ...
        fputs([[errLog description] cStringUsingEncoding:NSUTF8StringEncoding], file);
        fclose(file);
    }
}
...
//    CFStringRef remoteHostName = CFWriteStreamCopyProperty((CFReadStreamRef)[self readStream],  kCFStreamPropertySocketRemoteHostName);
}
...
- (void)fetchPACFile
{
	...
	if ([self isSynchronous]) {
		...
		if (PACRequest.responseStatusCode == 200 && ![PACRequest error] && [PACRequest responseString]) {
...
+ (void)hideNetworkActivityIndicatorAfterDelay
{
	[self performSelector:@selector(hideNetworkActivityIndicatorIfNeeeded) withObject:nil afterDelay:0.0];
}
