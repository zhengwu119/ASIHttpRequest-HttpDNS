...
- (NSURL *)convertDomainToIp{
    [domain release];
    NSURL *ipUrl = nil;
    domain = [[[self url] host] copy];
    ...
    if (![expression evaluateWithObject:domain] ) {
        ...
        if (ip != nil) {
            urlStr = [[[self url] absoluteString] stringByReplacingOccurrencesOfString:domain withString:ip];
            ipUrl = [NSURL URLWithString:urlStr];
        }else{
            ip = [domain copy];
        }
    }else{
    ...
    return ipUrl == nil ? [self url] : ipUrl;
}
...
- (void)main
{
    @try {
        ...
        NSURL *dPlusURL = [self useHTTPDNS] ? [self convertDomainToIp] : [self url];
        if (dPlusURL == nil) {
            [self failWithError:ASIUnableToCreateRequestError];
            return;
        }
        // Create a new HTTP request.
        request = CFHTTPMessageCreateRequest(kCFAllocatorDefault, (CFStringRef)[self requestMethod], (CFURLRef)dPlusURL, [self useHTTPVersionOne] ? kCFHTTPVersion1_0 : kCFHTTPVersion1_1);
        if (!request) {
            [self failWithError:ASIUnableToCreateRequestError];
            return;
        }
